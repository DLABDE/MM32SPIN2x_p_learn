...
/* 端口引脚宏定义,方便程序移植 */
#define  GPIO_OLED_CLK  				RCC_AHBPeriph_GPIOC
#define  OLEDGPIOx							GPIOC
#define  GPIO_OLED_SCLK_Pin     GPIO_Pin_6							/* D0 */
#define  GPIO_OLED_PIN_Pin     	GPIO_Pin_7							/* D1 */
#define  GPIO_OLED_RES_Pin     	GPIO_Pin_8							/* RES */
#define  GPIO_OLED_DC_Pin     	GPIO_Pin_9							/* DC */
...
#define OLED_RST_Clr() GPIO_ResetBits(OLEDGPIOx,GPIO_OLED_RES_Pin)	//RES RES => 接RES引脚
#define OLED_RST_Set() GPIO_SetBits(OLEDGPIOx,GPIO_OLED_RES_Pin)

#define OLED_DC_Clr() GPIO_ResetBits(OLEDGPIOx,GPIO_OLED_DC_Pin)	//DC  DC  => 接DC引脚 
#define OLED_DC_Set() GPIO_SetBits(OLEDGPIOx,GPIO_OLED_DC_Pin)

/* 使用4线串行接口时使用 */ 
#define OLED_SCLK_Clr() GPIO_ResetBits(OLEDGPIOx,GPIO_OLED_SCLK_Pin)//CLK D0  => 接D0引脚
#define OLED_SCLK_Set() GPIO_SetBits(OLEDGPIOx,GPIO_OLED_SCLK_Pin)

#define OLED_SDIN_Clr() GPIO_ResetBits(OLEDGPIOx,GPIO_OLED_PIN_Pin )//PIN D1  => 接D1引脚
#define OLED_SDIN_Set() GPIO_SetBits(OLEDGPIOx,GPIO_OLED_PIN_Pin )
  
/* PC0~7,作为数据线 */
#define DATAOUT(x) GPIO_Write(OLEDGPIOx,x);									//输出 
